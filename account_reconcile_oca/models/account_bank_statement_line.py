...
class AccountBankStatementLine(models.Model):
    ...
    def _compute_exchange_rate(self, data):
        ...
        if not currency.is_zero(amount):
            ...
            if account:
                ...
                raise UserError(_("No account for Expense Currency Exchange is found!"))
            reconcile_auxiliary_id += 1
        return reconcile_auxiliary_id
    ...
    def unreconcile_bank_line(self):
        self.ensure_one()
        reconcile_mode = "edit" if not self.reconcile_mode else self.reconcile_mode
        return getattr(
            self, "_unreconcile_bank_line_%s" % reconcile_mode
        )(  # self.reconcile_mode )(
            self.reconcile_data_info["data"]
        )
